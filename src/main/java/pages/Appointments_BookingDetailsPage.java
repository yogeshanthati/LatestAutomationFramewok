	...
	public void ValidateAppointmentDetailsInBookingDetailsPage() {
		...
		try {
			...
			if (isElementPresent(eleBookingDetails_ServiceIcon)) {
				...
				failed(driver, " failed to Validate the Service icon in Booking Details");
			}
			...
			if (Apoointment_Time.equals(GlobalVariables.SelectTimeSlotForAppointment)) {
				...
				failed(driver, "Failed To Validate the Appointment schedule time In Booking Details Expected As "
			...
			if (Apoointment_Date.replaceAll("\\s+", "")
				...
				failed(driver, "Failed To Validate the  Appointment schedule Date In Booking Details Expected As "
			...
			if (PhoneNumberInPatientDetails.equals(GlobalVariables.PhoneNumber)) {
				...
				failed(driver, "Failed To Validate the Phone Number Identity In Booking Details Expected As "
			...
			if (eleBookingDetails_Email.getText().equals(GlobalVariables.CreateUserName)) {
				...
				failed(driver, "Failed To Validate the Gender Identity in Booking Details Expected As "
	...
	public void ValidateRescheduleAppointmentInBookingUpdated() {
		...
		try {
			...
			if (Apoointment_Time.equals(GlobalVariables.SelectTimeSlotForAppointment)) {
				...
				failed(driver, "Failed To Validate the Appointment Rescheduled time In Booking Updated Expected As "
						+ (GlobalVariables.SelectTimeSlotForAppointment) + ",But Actual is" + Apoointment_Time);
			}
			...
			if (Apoointment_Date.replaceAll("\\s+", "")
				...
				failed(driver, "Failed To Validate the  Appointment Rescheduled Date In Booking Updated Expected As "
						+ (GlobalVariables.SelectTravelDate) + ",But Actual is" + Apoointment_Date);
			}
		} catch (Exception e) {
		...
		try {
			if (GlobalKeys.configData.get("PlatFormName").equalsIgnoreCase("Android")) {
				...
				WebElement btnDate = driver.findElement(By.xpath("//android.view.View[@content-desc='"
						+ getDateFormatToStringInAndroid(getFutureDate(2)) + "']"));
		...
		try {
			...
			if (GlobalKeys.configData.get("PlatFormName").equalsIgnoreCase("Android")) {
				...
				if (errSelectReasonForCancel.getText().replaceAll("\\s+", "")
					...
					failed(driver, "Failed To Validate the Select  Reason For Cancellation Error Message ,Expected As"
